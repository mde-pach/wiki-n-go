...
| 2026-06-14 | **Notifications are write-time push to systems we already run — no store, no index, no new third party** (Hub) | "Reverted / pending review / replied-to" each push at the moment they happen, routed by identity: `gh:` → a *visible* `@login` in the GitHub artifact (commit comment on rollback, PR comment on pending review, the reply body) so GitHub notifies natively; `wg:` → the IdP's existing SMTP via `POST /notify` (the IdP alone holds the address); anon → unreachable. The Engine holds **zero notification state** (`src/notify.ts`) — events are caught at write-time, not derived, so the rejected in-memory index never appears. Replies notify the **parent comment's author** (or topic author), found from the thread already being posted to. Needed a `wg:` comment marker (carrying the sub, not the handle) so a wg author is never mistaken for the GitHub login of the same name. All best-effort: a notification never blocks or fails the action. Backend-down alerting is left to in-site surfacing (editor publish errors + `/setup`), not an external monitor. The wg: email path's companion endpoint (`POST /notify`) is **implemented in the IdP app** (`accounts/src/index.ts`, same monorepo) — verifies a bearer, resolves `sub→email`, sends via the existing SMTP; wire with `NOTIFY_TOKEN` (IdP) + `IDP_MAIL_URL`/`IDP_MAIL_TOKEN` (Engine). Contract: `worker/NOTIFY.md` |
| 2026-06-14 | **Custom domains = a verified `domain` on the tenant line + `resolveHost` matching off-platform hosts; CNAME-verified via DoH; cert is the one infra step** (Hub) | A BYO owner points `wiki.mybrand.com` at their wiki by adding a CNAME to `<name>.<platformHost>`; `POST /domain` records it on the tenant (registry `domain` field, last-write-wins) **only after** the owner is verified (registry `owner`) and the CNAME is confirmed via DNS-over-HTTPS — so a host can't be hijacked onto someone else's repo. `resolveHost` now matches a host *outside* the platform domain against `latestByDomain`; the frontend `bootTenant` resolves any non-apex host (an unregistered one just 404s → baked config). The **TLS certificate** for the custom host is the single coordinated infra step (Traefik per-domain HTTP-01) — code-side everything else is in place and tested. No-DB preserved (the mapping is a registry line) |
| 2026-06-14 | **BYO "use this template" is a deep-link to GitHub's tokenless generate flow, not a server-side repo create** (Hub) | Creating a repo in a *user's own* account needs the user's credentials, which the in-site invariant forbids us from asking for. GitHub's `…/<template>/generate` web flow lets the already-signed-in user make their own copy with zero token to paste — same pattern as the App-install deep-link. So `/create` (byo lane) links to `templateGenerateUrl(config.templateRepo)`; the user generates `their/repo`, installs the App, and enters it. The starter template (reader + Pages workflow + seed content) is an operator-published repo (`PUBLIC_TEMPLATE_REPO`, default `wikigit/template`) marked as a GitHub template — server-side generation into the org would be the managed lane, not BYO |
| 2026-06-14 | **Export/transfer bridge is two explicit steps, gated on the registry `owner`, re-pointing only after the new repo is App-installed** (Hub) | "Move to your own GitHub" can't be one synchronous call: a GitHub repo transfer must be **accepted by the new owner out-of-band**, and the org-wide content-App install doesn't follow the repo out of the org. So `POST /transfer` (platform App, Administration write) only **initiates** the move; `POST /transfer/complete` re-points the registry (`repointTenant`, last-write-wins, lane→byo) **only once `repoInstallationId` confirms the content App is installed on `<owner>/<name>`** — otherwise the subdomain would resolve to a repo the Engine can't serve. Both are operator-global and authorised by the registry `owner` (not a maintainer check against the about-to-move repo). Reading continues via the CDN through the brief window; editing resumes when the move is finished. This is the "Quick lane is never a trap" promise |
| 2026-06-14 | **Guided first-run is a live-state checklist on `/setup`, not stored to-do state** (Hub) | "Write your first page → invite an editor → set who can edit" each derive `done` from a live signal (link-graph page count, config-maintainers ∪ granted editors, sign-in availability) rather than a per-wiki flag — so there's nothing to persist or mark done, it can't drift from reality, and it reuses the existing `.setup-list` chrome. Completed steps drop their call-to-action. All sources are best-effort (a mid-setup wiki just shows steps pending) |
