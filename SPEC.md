...
| 2026-06-06 | **Edit-conflict detection keys on the file's git blob SHA, not the repo commit SHA**: the editor computes the loaded page's blob SHA client-side and sends it; the Worker 409s if it ≠ the current blob (or the page is gone). Missing base = no check (back-compatible) | The blob SHA is content-addressed, so it only conflicts when *that page* actually changed — unrelated commits don't false-positive, and no extra Worker fetch is needed (`getCurrentFile` already returns it). The notice is non-destructive: the textarea keeps the user's text (persisted as a draft), so reloading re-bases on the latest and reapplies their edit |
