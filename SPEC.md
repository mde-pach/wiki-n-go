...
- [x] ✅ **Merge/split** (`worker/src/handlers/lifecycle.ts`, `POST /merge` + `/split`): built from the **move
      primitive** (gated direct commits + redirect stub) and gated **like a normal edit** (Turnstile + bans +
      rate-limit via `resolve()`, AbuseFilter `runFilters`, trust-tier check on every affected page — insufficient
