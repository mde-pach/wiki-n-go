...
- [x] ✅ **New-Pages queue + deletion + Page Curation toolbar** — `/admin` **New pages** tab lists recently created
      ...
      one place — approve (patrol) · tag (→ editor) · message author (→ talk) · contributions · roll back · propose-delete,
      with patrol state + the revert-risk badge inline; it sits on each New-pages row **and** on any page's read view,
      with optimistic UI over the existing patrol/rollback/delete endpoints (no new Worker route).
...
| 2026-06-07 | **Page Curation toolbar (FEATURES §M) is a pure frontend overlay over the existing patrol/rollback/delete endpoints — no new Worker route; actions with no endpoint link to the in-site flow** | The New-pages queue, patrol, rollback and delete endpoints already existed (M6), so the reviewer toolbar is a UX layer, not new backend: one reusable `PageCuration` Solid component triages a page in one place (approve = `POST /patrol`, roll back = `POST /rollback`, propose-delete = `POST /delete`) with optimistic UI + error handling, surfacing patrol state + the revert-risk badge inline. It resolves a page's latest sha + patrol bit from `GET /patrol-status` and enriches author/risk from the recent-changes feed (`curation.ts`), so it works from a slug alone on the read view *and* from a handed-in change row on the New-pages tab (no refetch). Actions that genuinely lack an endpoint **link to the existing in-site flow** rather than add a route — **tag** → the editor (add maintenance tags as frontmatter), **message author** → the page's talk, **contributions** → the author's `/changes?author=` (anon) or `/user/<login>` (signed-in) — keeping the single-Worker invariant. Maintainer-gated from `whoami` (same pattern as the other `/admin` surfaces); renders nothing for everyone else |
