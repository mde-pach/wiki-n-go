...
| S8 | **Body text is too small vs Wikipedia** — the base reading font is undersized; bump it and re-check the other type-scale steps (headings, captions) for the same gap. Done: `--text-base` → 1rem (16) in both skins (wiki 14→16 = WP's post-2023 size; wikigit 15→16), with headings, captions and `--measure` rescaled proportionately, chrome (`--text-ui`) left unscaled; pure tokens (pre-paint, no blink) and the Appearance text-size steps stay distinct/ordered (14/16/18). `85021b0` | 🐛 | ✅ | P1 | §H, §D |
| S9 | **Collapse/toggle arrows are too small vs Wikipedia** — the per-heading section caret and the chrome chevrons (Tools dropdown, mobile TOC, review/discussion expanders) read as tiny next to Wikipedia's. Enlarged + emboldened: `.section-toggle` 0.7→1em with a 0.62em/2.5px caret, and the `.chev`/`.rv-caret`/`.topic-caret`/mobile-TOC chevrons to 18–20px at `stroke-width` 2.1. CSS/token-only, no markup or behaviour change. | 🐛 | ✅ | P1 | §C, S2, S8 |
| S10 | **Body text slides under the floating infobox** — at wider columns (where the short intro clears before the tall card does), list items beside the card's lower edge rendered *under* it. Root cause: the card was a separate island (`Infobox.tsx`) outside the prose flow, so the float's clearance/intrusion was unreliable for nested block content (paragraphs wrapped, list items didn't). Fixed by baking the card into the article body as its real first child (`lib/infobox` → `infoboxHtml`, prepended in `WikiPage`; the `<Infobox>` island is gone from the read page + `Route404`), so it floats inside the prose flow MediaWiki-style; top-level lists get `display: flow-root` so any that would straddle the card drop cleanly below it. Also zeroed a stray `top:` the sticky base left on the now-relative card (it shifted the `float:none` mobile card down over the lead paragraph). Verified 0 overlap at standard/wide/mobile. | 🐛 | ✅ | P1 | §D, S3, S7 |
...
| T6 | **Section-scoped editing** — a section `[edit]` should open a focused editor for *that section only* (a purpose-built in-page edit surface), not the whole-page editor. Design it as the generic pattern for in-page content editing (header card, infobox row, section body all reuse it). Done: a heading `[edit]` opens `FocusedEditor` **in place** (WikiPage portals it right under the heading), seeded with the section slice; on save it splices back + reconstructs the whole doc and submits through the **same `submitEdit` pipeline** (PR/trust/Turnstile/filter/diff-preview/deterministic-branch — no second write path, Worker untouched). `FocusedEditor` is generic (SectionSpan + source + `reconstruct`) for reuse by a header card / infobox row. `findSection` is now level-aware (carries deeper subsections) + a pure `spliceSection`, both unit-tested; "Edit whole page →" escape hatch kept. `4d0fe1e` | ✨ | ✅ | P1 | §G, S3 |
| T7 | **Edit page feels bloated** for a page with modest content — too much chrome around little data. Restructure the edit layout to surface the actual content more efficiently (denser, less framing). Done: properties panel collapsed by default (open on create; "N set" badge keeps populated metadata discoverable), the framed "Publish your change" card → a compact `.publish-bar` (summary + Publish/Cancel on one row, attribution beneath), shorter subtitle, tighter token-based spacing, taller editor panes; draft/diff/summary/Turnstile/section deep-link all unchanged (CSS + structure only). `c6e34a6` | 🐛/✨ | ✅ | P1 | §G |
| T8 | **Edit preview doesn't run md plugins** (e.g. Mermaid renders as raw fence in the preview viewer). Preview must use the **same renderer as the published page** so extensions render identically. Fixed: a `MutationObserver` on the preview re-runs the published `renderMermaid` pass (debounced) whenever its HTML changes, so `\`\`\`mermaid` fences render as diagrams in-preview. `9d59736` | 🐛 | ✅ | P1 | §D, T5 |
...
| W4 | **Clicking a non-default-language page that exists behaves as "page doesn't exist"** — spotted on the FR *Démarrer* (Get started) link: the target page is present but the UI treats it as a redlink/404. Interlanguage link/route resolution drops the language prefix or the `translationKey` lookup. **Real cause (owner repro):** Cloudflare Pages 308-strips a trailing `/index` segment, so the home's tabs `/edit/index`, `/history/index`, `/talk/index` (and language homes `/edit/fr/index`) redirected to `/edit/` etc. and 404'd. Fixed: `staticPaths()` collapses `index`→bare / `fr/index`→`fr` like the read route (home editor builds at `/edit`, fr at `/edit/fr`), routes map the param back via `contentSlugForRoute`, and a new `viewHref()` builds the collapsed links everywhere (tabs · Toc · PageMeta · section `[edit]` · history revert · curation · focused-editor · SPA fallback); `parseRoute` mirrors it. Regression-tested. (Round-1 `readHref` href fix `8182535` was a real but separate latent bug.) `95796dc` | 🐛 | ✅ | P1 | §B, W3 |
| W5 | **"Add language" suggests irrelevant languages** (e.g. Dutch appears unprompted). Instead surface the **languages already present in the wiki** (those are the ones likely to be extended), and offer a distinct **"translate to a new language"** path for a language not yet in the wiki. Fixed: `LangBar` now lists existing translations as switch links, languages already present elsewhere in the wiki as inline "add", and languages not yet anywhere under a distinct "Translate to a new language" section (SSR-seeded by `wikiLanguages()`, reconciled live against the link graph). `daf796c` | 🐛/✨ | ✅ | P1 | §B, W3 |
| W6 | **Header goes greyscale when the menu opens** — opening the main menu desaturates the header chrome (unintended overlay/filter bleed). Fixed (same root cause as W7): the overlay was clipped to the backdrop-filtered `.header`, veiling only it; portaling to `document.body` makes the scrim cover the page uniformly. `9b6dd56` | 🐛 | ✅ | P2 | §A |
| W7 | **Open nav menu doesn't close on outside click** — clicking outside the open navigation header should dismiss it (`MainMenu`). Fixed: `.header`'s `backdrop-filter` made it the containing block for the fixed overlay, clipping the scrim to the header strip so body clicks never hit it. Portal the drawer to `document.body`; verified the scrim covers the viewport and an outside click closes it. `9b6dd56` | 🐛 | ✅ | P1 | §A |
| W8 | **Two inline sign-in buttons → one button + a provider modal** — when both providers were enabled the header showed bare `GitHub` / `Wikigit` buttons side by side. Now a single **Sign in** button opens a small modal ("Continue with GitHub / Continue with Wikigit", each with its mark) — the familiar OAuth-provider pattern. The button also **renders from first paint** regardless of `/auth/status` (it only hides if the Worker *positively* reports auth disabled), so it no longer blinks in after the round-trip. `AuthButton` (Portal modal, scrim-button dismiss) + `.signin-modal`/`.provider-*` styles. Follow-up: the first cut read the session synchronously, so a *signed-in* client tried to hydrate the avatar over the server-rendered button and threw — freezing the chrome on "Sign in" (and the modal dead). Server and hydration now render the same button; the session + providers resolve in `onMount` (signed-out: stable button, no blink; signed-in: flips to the avatar right after hydration), and the Portal is gated on a client-only `mounted` flag so it never enters the hydration tree. Follow-up 2: killed the remaining signed-in flash entirely. Cookie+SSR was rejected (pages are static/CDN-shared — per-user SSR is impossible on static routes and would leak the avatar through the `s-maxage` edge cache). Instead, mirroring `ThemeBoot`: a new `AuthBoot.astro` pre-paint inline script decodes the stored session and paints the avatar (or Sign-in button) into `#auth-pre` *before first paint*, and `AuthButton` is now `client:only` (reads the session synchronously, renders the right chrome from its first render, removes `#auth-pre` on mount). Signed-in users see their avatar at first paint with zero flash; nothing in the cached HTML is per-user. | 🐛/✨ | ✅ | P1 | §A, R4, M10 |
| W9 | **Language picker only offered the 3 configured languages** — "translate this page" was a free-text code box and the switcher dropped any language outside `config.languages`. Now backed by `iso-639-1` (new `lib/languages`): the `/new` translate flow is a `<select>` of all 183 ISO 639-1 languages (native names, configured ones first), `LangBar`/`i18n` resolve any code's name through the lib and list every sibling language, and the generic "translate into another language" path is always open. `config.languages` stays the *featured/ordered* set, not a hard limit. | ✨ | ✅ | P1 | §B, W3, W5 |

## X. Page-preview hovercards (`lib/previews`)
| # | Item | Type | St | Pri | Ref |
|---|---|---|---|---|---|
| X1 | **Clicking a link before the hovercard appears strands the popover** — it pops in late at the top-left / out of bounds of the freshly opened page. Cancel the pending preview (and any in-flight fetch) on navigation so it never renders after the click. Fixed: cancel on link click and on `astro:before-swap`/`pagehide`, and refuse to render for a detached link (`isConnected`); verified no stranded card after an early-click SPA navigation. `34801d1` | 🐛 | ✅ | P1 | §D |
| X2 | **"Read full page" inside the hovercard isn't clickable** — the primary action of the popover does nothing; make it a working link to the article. Fixed: the card body is now an `<a>` to the article's `readHref` (the whole card click-throughs). `34801d1` | 🐛 | ✅ | P1 | §D |
...
| Y2 | **No discoverable route to the setup/config page** — the Cloudflare-connect (and other) config controls live in `Setup.tsx` / `pages/setup.astro` but there's no entry point from the app chrome to reach them. Add a discoverable link (owner/admin menu). Fixed: a **Setup** link sits beside Admin in the footer-tools row (and the `Route404` fallback) via a new `setupHref`. `f5cd435` | 🐛/✨ | ✅ | P1 | §A, SPEC M9 |
